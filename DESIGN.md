...
- New/changed components:
  ...
  - 홈 화면의 퀵스타트 카드.
  - 오늘의 투자 브리핑 카드.
  - 홈 데이터 신뢰 상태(리포트·신호 기준일, 데이터 상태, 시스템 검증 결과)와 우선순위가 연결된 갱신 안내.
  - 시장 기준일, 마지막 수집 완료 시각, 마지막 분석 결과 생성 시각을 분리한 홈 신뢰 상태 요약.
  - 오늘 볼 만한 종목 카드.
  ...
  - 데이터 보기의 최대 3종목 공통 거래일 비교 차트와 시작일 100/원가격 기준 전환.
  - 후보 카드의 직접 선택 버튼.
  - 선택 상세 영역 스크롤 anchor.
  ...
  - 백테스트 실험 설정 패널. 전략별로 의미 있는 제한된 입력만 보여주고, 기본/실험 상태, 허용 범위, 신호에 미치는 효과, 기본값 되돌리기를 함께 제공한다.
  - 복수 전략 조합 패널. AND를 기본으로 하며 OR·가중치 방식을 명시적으로 선택하고, 선택한 규칙·가중치·구성 전략별 신호를 결과와 저장 이력에서 다시 확인할 수 있게 한다.
  - 다종목 포트폴리오 집계 패널. 전략별 결과를 섞지 않고, 동일 비중·리밸런싱 없음·공통 거래일이라는 가정과 공통 시작일 재기준화 이유를 먼저 밝힌 뒤 평가금액 곡선, 비중·기여도·제외 사유를 보여준다.
  - 사이드바 화면 설정 패널. 종목·기간·검색/정렬/필터·차트 표시 기준을 현재 브라우저 세션의 복원 지점으로 저장·적용·초기화하며, 저장 범위와 세션 한정을 같은 패널에서 설명한다.
- Variants and states:
...
- Microcopy rules:
  ...
  - 홈의 다음 행동은 시스템 검증 실패, 데이터 기준일 문제, 수집 실패, 리포트 확인, 백테스트 순서로 우선순위를 표시한다.
  - `시장 기준일`은 리포트와 전략 신호가 같은 거래일일 때만 단일 날짜로 표시하고, 다르면 불일치와 각 기준일을 함께 표시한다.
  ...
  - 실험 설정은 ‘과거 데이터에서만 확인하는 값’으로 표현하고, 설정값·기본값 여부를 결과 카드와 저장 메타데이터에서 다시 확인할 수 있게 한다.
  - 복수 전략 조합은 ‘전략을 고르는 추천’이 아니라 비교 규칙으로 설명한다. 기본 AND의 관망 기준, OR의 매수·매도 충돌 관망, 가중치 합산의 0점 관망을 화면에 그대로 밝히고, 가중치 1.0은 성과 예측이 아닌 중립 비교 시작값으로 표현한다.
  - 포트폴리오 집계는 실제 비중 추천이나 리밸런싱 안내가 아닌 과거 결과의 비교 기준으로 표현한다. 포함 종목 수, 제외 사유, 공통 거래일 및 공통 시작일 재기준화 가정을 수익률과 같은 화면에서 제공한다.
  - 화면 설정은 탭 이동 중 자동으로 유지되는 현재 상태와 사용자가 명시적으로 저장한 복원 지점을 구분해 설명한다. 탭 전환으로 Streamlit 위젯 상태가 정리되어도, 현재 탭의 비어 있는 설정은 세션 초안에서만 다시 채우고 사용자가 방금 바꾼 값은 덮어쓰지 않는다. 현재 브라우저 세션에만 보관하고 서버 파일·DB·다른 사용자에게 저장하지 않는다는 범위를 표시한다.
  - 설정 저장 범위에는 종목·기간·검색/정렬/필터·차트 표시 기준만 포함한다. 백테스트 결과, 실행 상태, 실험 파라미터와 조합 가중치는 저장하거나 변경하지 않는다.
