...
def test_market_report_generator_builds_and_saves_summary_report():
    ...
    request = MarketReportRequest(
        ...
        investor_summary_filename="005930_20260410.csv",
    )
    ...
    assert report.iloc[0]["investor_date_source"] == "summary:column:stck_bsop_date"
...
def test_market_report_generator_prefers_canonical_symbol_name_over_stock_info_fallback():
    ...
    request = MarketReportRequest(
        ...
        investor_summary_filename="000660_20260410.csv",
    )
...
def test_market_report_parses_numeric_kis_dates_after_csv_round_trip() -> None:
    test_dir = make_test_dir("market_report_numeric_kis_dates")
    raw_storage = CsvStorage(test_dir / "raw")
    processed_storage = CsvStorage(test_dir / "processed")
    generator = MarketReportGenerator(raw_storage=raw_storage, processed_storage=processed_storage)
    request = MarketReportRequest(
    ...
    processed_storage.save(
        "golden_cross_signals",
        request.signal_filename,
        pd.DataFrame([{"date": 20260410, "signal": "buy", "signal_reason": "numeric KIS date"}]),
    )
    raw_storage.save(
        "investor_daily",
        request.investor_filename,
        pd.DataFrame(
            [
                {
                    "stck_bsop_date": 20260410,
    ...
    assert indicator_frame.iloc[0]["date"] == pd.Timestamp("2026-04-10")
    assert signal_frame.iloc[0]["date"] == pd.Timestamp("2026-04-10")
    assert investor_frame.iloc[0]["stck_bsop_date"] == 20260410
    assert report.iloc[0]["reference_date"] == "2026-04-10"
    assert report.iloc[0]["investor_date"] == "2026-04-10"


def test_market_report_rejects_undated_legacy_investor_detail_without_summary() -> None:
    test_dir = make_test_dir("market_report_undated_legacy_investor")
    raw_storage = CsvStorage(test_dir / "raw")
    processed_storage = CsvStorage(test_dir / "processed")
    generator = MarketReportGenerator(raw_storage=raw_storage, processed_storage=processed_storage)
    raw_storage.save(
    ...
    raw_storage.save(
        "investor_daily_summary",
        "005930_20260412.csv",
        pd.DataFrame(columns=["stck_bsop_date"]),
    )
    request = MarketReportRequest(
        "005930",
        "indicator.csv",
        "signal.csv",
        "005930_20260412.csv",
        investor_summary_filename="005930_20260412.csv",
    )

    with pytest.raises(MarketReportDataError, match="investor_daily_summary.*refresh"):
        generator.load_investor_frame(request)


def test_market_report_uses_latest_observation_from_reverse_order_investor_summary() -> None:
    test_dir = make_test_dir("market_report_reverse_investor_summary")
    raw_storage = CsvStorage(test_dir / "raw")
    generator = MarketReportGenerator(
    ...
    raw_storage.save(
        "investor_daily_summary",
        "005930_20260412.csv",
        pd.DataFrame([{"stck_bsop_date": 20260410}, {"stck_bsop_date": 20260409}]),
    )
    request = MarketReportRequest(
        "005930",
        "indicator.csv",
    ...
    assert investor_frame.iloc[0]["stck_bsop_date"] == pd.Timestamp("2026-04-10")
    assert investor_frame.iloc[0]["__source_date_hint"] == "summary:column:stck_bsop_date"


def test_market_report_uses_historical_summary_values_at_older_common_reference_date() -> None:
    test_dir = make_test_dir("market_report_historical_investor_summary")
    raw_storage = CsvStorage(test_dir / "raw")
    generator = MarketReportGenerator(
        raw_storage=raw_storage,
        processed_storage=CsvStorage(test_dir / "processed"),
    ...
    raw_storage.save(
        "investor_daily",
        "005930_20260412.csv",
        pd.DataFrame([{"frgn_ntby_qty": 999, "orgn_ntby_qty": 999, "prsn_ntby_qty": -1998}]),
    )
    raw_storage.save(
        "investor_daily_summary",
        "005930_20260412.csv",
        pd.DataFrame(
            [
                {"stck_bsop_date": 20260410, "frgn_ntby_qty": 100, "orgn_ntby_qty": 50, "prsn_ntby_qty": -150},
                {"stck_bsop_date": 20260409, "frgn_ntby_qty": -30, "orgn_ntby_qty": -20, "prsn_ntby_qty": 50},
            ]
        ),
    )
    request = MarketReportRequest(
    ...
    report = generator.generate_report(
        request=request,
        indicator_frame=pd.DataFrame([{"date": "2026-04-09", "close": 100}]),
        signal_frame=pd.DataFrame([{"date": "2026-04-09", "signal": "hold"}]),
        investor_frame=generator.load_investor_frame(request),
        stock_info_frame=pd.DataFrame(),
    )

    assert report.iloc[0]["reference_date"] == "2026-04-09"
    assert report.iloc[0]["investor_date"] == "2026-04-09"
    assert report.iloc[0]["foreign_net"] == -30.0
    assert report.iloc[0]["institutional_net"] == -20.0
    assert report.iloc[0]["investor_date_source"] == "summary:column:stck_bsop_date"


def test_market_report_rejects_ambiguous_undated_multirow_investor_detail() -> None:
    test_dir = make_test_dir("market_report_ambiguous_investor_detail")
    raw_storage = CsvStorage(test_dir / "raw")
    generator = MarketReportGenerator(
        raw_storage=raw_storage,
        processed_storage=CsvStorage(test_dir / "processed"),
    )
    filename = "005930_20260412.csv"
    raw_storage.save(
        "investor_daily",
        filename,
        pd.DataFrame([{"frgn_ntby_qty": 10}, {"frgn_ntby_qty": 20}]),
    )
    raw_storage.save(
        "investor_daily_summary",
        filename,
    ...
    request = MarketReportRequest(
        ...
        "signal.csv",
        filename,
        investor_summary_filename=filename,
    )

    with pytest.raises(MarketReportDataError, match="multiple rows.*cannot be aligned safely"):
        generator.load_investor_frame(request)


def test_market_report_prefers_partial_dated_summary_values_over_undated_detail() -> None:
    test_dir = make_test_dir("market_report_partial_investor_summary")
    raw_storage = CsvStorage(test_dir / "raw")
    generator = MarketReportGenerator(
        raw_storage=raw_storage,
        processed_storage=CsvStorage(test_dir / "processed"),
    ...
    raw_storage.save(
        "investor_daily_summary",
        filename,
        pd.DataFrame([{"stck_bsop_date": 20260410, "frgn_ntby_qty": 25}]),
    )
    request = MarketReportRequest(
        "005930",
        "indicator.csv",
    ...
    report = generator.generate_report(
        request=request,
        indicator_frame=pd.DataFrame([{"date": "2026-04-10", "close": 100}]),
        signal_frame=pd.DataFrame([{"date": "2026-04-10", "signal": "hold"}]),
        investor_frame=generator.load_investor_frame(request),
        stock_info_frame=pd.DataFrame(),
    )

    assert report.iloc[0]["foreign_net"] == 25.0
    assert report.iloc[0]["institutional_net"] is None
    assert report.iloc[0]["personal_net"] is None
    assert report.iloc[0]["investor_flow"] == "unknown"


def test_market_report_uses_dated_summary_values_when_investor_detail_is_empty() -> None:
    test_dir = make_test_dir("market_report_empty_detail_with_summary")
    raw_storage = CsvStorage(test_dir / "raw")
    generator = MarketReportGenerator(
        raw_storage=raw_storage,
        processed_storage=CsvStorage(test_dir / "processed"),
    ...
    filename = "005930_20260412.csv"
    raw_storage.save("investor_daily", filename, pd.DataFrame())
    raw_storage.save(
        "investor_daily_summary",
        filename,
        pd.DataFrame(
    ...
    assert investor_frame.iloc[0]["stck_bsop_date"] == 20260410
    assert investor_frame.iloc[0]["frgn_ntby_qty"] == 120
    assert investor_frame.iloc[0]["__source_date_hint"] == "summary:column:stck_bsop_date"
