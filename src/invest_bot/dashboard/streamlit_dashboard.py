...
def _render_reports_tab(snapshot, service: DashboardDataService) -> None:
    ...
    for preview in report_previews:
        _render_market_report_card(preview, service)

    if signal_previews:
        st.markdown("<div style='height:0.5rem'></div>", unsafe_allow_html=True)
        st.markdown("#### 최신 골든크로스 신호")
        signal_columns = st.columns(2)
        for index, preview in enumerate(signal_previews[:4]):
...
def _render_market_report_card(preview: DatasetPreview, service: DashboardDataService) -> None:
    frame = _read_preview_frame(preview.path)
    if frame.empty:
        return
    row = frame.iloc[-1]
