...
class DashboardDataService:
    ...
    def render_html(self, message: str = "", message_type: str = "info") -> str:
        snapshot = self.build_snapshot()
        return self._render_snapshot(snapshot, message=message, message_type=message_type)
        ...
        for dataset_dir in sorted(path for path in root.iterdir() if path.is_dir()):
            ...
            previews.append(
                DatasetPreview(
                    ...
                    column_help_html=self._build_column_help(list(enriched.columns), recommended),
                    column_selector_html=self._build_column_selector(dataset_dir.name, list(enriched.columns), recommended),
    ...
    def _enrich_frame(frame: pd.DataFrame, symbol: str, symbol_name: str) -> pd.DataFrame:
        ...
        rename_targets = {old: new for old, new in rename_map.items() if old in enriched.columns and new not in enriched.columns}
        if rename_targets:
    ...
    def _build_column_help(self, columns: list[str], recommended_columns: list[str]) -> str:
        if not columns:
            return '<div class="empty-text">설명할 컬럼이 아직 없습니다.</div>'
...
<section class="controls-panel">
  <div class="controls-text">
    <h4>표에 표시할 컬럼 선택</h4>
    <p>처음에는 추천 컬럼만 켜두고, 필요할 때만 하나씩 추가해서 보는 편이 읽기 쉽습니다.</p>
  </div>
...
<th data-column="{escape(column)}" title="{escape(meta.description)}">
  ...
  <div class="th-key">{escape(column)}</div>
    ...
    def _build_highlight(self, dataset_name: str, frame: pd.DataFrame) -> str:
        if frame.empty:
            return ""
        if dataset_name == "golden_cross_signals":
            return self._build_golden_cross_highlight(frame)
        if dataset_name == "market_reports":
            return self._build_market_report_highlight(frame)
        return ""

    def _build_golden_cross_highlight(self, frame: pd.DataFrame) -> str:
        latest = frame.iloc[-1]
...
<section class="highlight-panel">
  <div class="case-row-head">
    ...
    {self._render_state_badge(signal)}
  </div>
  ...
  <div class="guide-grid">
    ...
    def _build_market_report_highlight(self, frame: pd.DataFrame) -> str:
        latest = frame.iloc[-1]
        report_date = str(latest.get("date", "")).strip()
        summary = str(latest.get("summary", "")).strip() or "아직 생성된 요약 문장이 없습니다."
        final_opinion = str(latest.get("final_opinion", "hold")).strip() or "hold"
        trend_state = str(latest.get("trend_state", "unknown")).strip() or "unknown"
        rsi_state = str(latest.get("rsi_state", "unknown")).strip() or "unknown"
        volume_state = str(latest.get("volume_state", "unknown")).strip() or "unknown"
        investor_flow = str(latest.get("investor_flow", "unknown")).strip() or "unknown"
        signal = str(latest.get("golden_cross_signal", "hold")).strip() or "hold"
        reason = str(latest.get("golden_cross_reason", "")).strip()

        stat_cards = [
            ("최종 판단", self._state_label(final_opinion)),
            ("추세 상태", self._state_label(trend_state)),
            ("RSI 상태", self._state_label(rsi_state)),
            ("거래량 상태", self._state_label(volume_state)),
            ("수급 상태", self._state_label(investor_flow)),
            ("골든크로스", self._state_label(signal)),
        ]
        cards_html = "".join(
            f'<article class="guide-card"><h4>{escape(title)}</h4><p>{escape(value)}</p></article>'
            for title, value in stat_cards
        )
        reason_html = f'<p class="highlight-copy">전략 판단 근거: {escape(reason)}</p>' if reason else ""

        return f"""
<section class="highlight-panel report-highlight">
  <div class="case-row-head">
    <div>
      <h4 class="info-title">최신 시장 리포트</h4>
      <div class="dataset-key">{escape(report_date)}</div>
    </div>
    {self._render_state_badge(final_opinion)}
  </div>
  <p class="highlight-copy">{escape(summary)}</p>
  {reason_html}
...
<article class="case-row">
  <div class="case-row-head">
    ...
    <span class="{self._badge_class(case.status)}">{escape(case.status)}</span>
  </div>
  {detail}
</article>
                """.strip()
...
<section class="section">
  <div class="section-header">
    <div><h2>테스트 결과</h2><p>최근 저장한 pytest 결과입니다. 실패한 테스트가 무엇인지 빠르게 확인할 수 있습니다.</p></div>
    <span class="badge">pytest</span>
  </div>
  {summary_cards}
  <div class="card">
    ...
    <div class="guide-grid">
      <article class="guide-card"><h4>이 영역은 무엇인가</h4><p>테스트가 몇 개 통과했고 몇 개 실패했는지 한 번에 보는 영역입니다.</p></article>
      <article class="guide-card"><h4>왜 보는가</h4><p>긴 로그를 다 읽지 않아도 어디에서 문제가 났는지 빠르게 찾을 수 있습니다.</p></article>
      <article class="guide-card"><h4>처음에는 무엇을 볼까</h4><p>실패/에러 개수를 먼저 보고, 아래에서 failed나 error 상태의 테스트를 확인해 보세요.</p></article>
    </div>
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .controls-panel {{ padding:16px; margin-bottom:16px; background:var(--sand); }}
...
<section class="section">
  ...
  <div class="guide-grid">
    ...
    def _render_flash_message(self, message: str, message_type: str) -> str:
        if not message:
            return ""
        safe_type = message_type if message_type in {"success", "error", "info"} else "info"
        title = {
            "success": "작업이 완료되었습니다",
            "error": "확인이 필요한 문제가 있습니다",
            "info": "안내",
        }[safe_type]
        return f"""
<section class="flash-card {safe_type}">
  <h3>{escape(title)}</h3>
  <p>{escape(message)}</p>
</section>
        """.strip()
    ...
    def _badge_class(self, status: str) -> str:
        ...
        if normalized in {"buy", "passed", "success", "bullish", "supportive", "strong", "active"}:
            suffix = "passed"
        elif normalized in {"sell", "failed", "error", "bearish", "overbought"}:
            suffix = "failed"
        else:
            suffix = "skipped"
        return f"case-badge {suffix}"

    def _render_state_badge(self, value: str) -> str:
        return f'<span class="{self._badge_class(value)}">{escape(self._state_label(value))}</span>'

    def _state_label(self, value: str) -> str:
        normalized = value.strip().lower()
        return self.STATE_LABELS.get(normalized, value)
