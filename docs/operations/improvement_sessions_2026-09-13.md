...
5. 동시에 실행하는 전문 작업은 최대 1개다. 데이터와 퀀트 작업을 마무리했으며, 다음 진행 단위는 대시보드 작업이다.
...
## 퀀트 순차 작업 완료 (2026-09-16)

- 이전 통합 8개 커밋은 메시지 정리 후 `c2566ff`까지 main에 푸시했다. 이번 후속 작업 브랜치는 `codex/quant-report-followup-20260916`이다.
- 기존 퀀트 작업 하나만 Sol/medium으로 재개했으며, 후속 커밋 `2530743`, `56f8d67`, `e9cbe8c`의 변경을 통합했다. 다른 역할은 대기 상태다.
- CSV 숫자형 `YYYYMMDD`를 실제 날짜로 읽고, 주말 요청 파일명의 날짜를 수급 관측일로 사용하지 않는다.
- 날짜 없는 상세 수급은 matching summary의 날짜별 수급값을 우선 사용한다. 일부 수급 컬럼이 없거나 상세 frame이 비어 있어도 정상 summary 값을 보존한다.
- 날짜만 있는 summary는 단일 상세 행과만 연결한다. 날짜 없는 다중 상세 행이나 관측일을 확정할 수 없는 입력은 명시적으로 거부한다.
- 검증: 역할별 표적 24 passed, 통합본의 격리된 기본 suite 379 passed / PostgreSQL 1 deselected. 기존 Alembic 설정 경고 6건이며 실패는 없다. 이번에는 PostgreSQL·외부 API·실거래 검증을 실행하지 않았다.
- 다음 작업: 대시보드 구조·조회 개선. 아직 재개하지 않았다.
