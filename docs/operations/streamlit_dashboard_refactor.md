...
### 2026-08-24 / Home trust-aware next actions

- 배경
  - 홈의 리포트·신호 기준일 신뢰 상태와 다음 행동 카드가 서로 다른 기준으로 판단되어, 데이터 확인이 필요한 경우에도 리포트 확인을 먼저 제안할 여지가 있었음
- 변경 사항
  - `streamlit_overview.py`
    - 데이터 준비 상태와 시스템 검증 상태를 분리해, 테스트 실패가 기준일 문제를 내부적으로 가리지 않도록 정리
    - 다음 행동을 `시스템 검증 → 데이터 갱신 → 수집 실패 확인 → 투자 리포트 → 백테스트` 우선순위로 구성
    - 리포트·신호 기준일이 확인되고 테스트/수집 상태가 정상일 때에만 리포트 확인과 백테스트를 제안
  - `tests/test_streamlit_dashboard.py`
