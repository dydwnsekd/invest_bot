...
- 사이드바가 DB 스냅샷을 사용하는 화면에서도 CSV 경로를 표시하던 문제를 수정했다. 실제 저장 방식이 데이터베이스이면 `원본·분석 데이터: 데이터베이스 스냅샷`을, 파일 저장 방식이면 해당 원본·분석 경로를 보여준다. DB 접속 주소는 화면에 표시하지 않는다.
- 관련 테스트 159개가 통과했고, 실제 브라우저의 사이드바에서도 DB 안내가 정상 표시되는 것을 확인했다. 변경은 사용자 검토를 위해 커밋하지 않았다.
- 관심종목 반복 조회 분석과 최신 코드 배포는 별도 후속 작업이다.
...
- 당시 변경은 사용자 검토를 위해 커밋하지 않았다. 사이드바 데이터 위치 안내는 위 후속 수정에서 해결했고, 관심종목 반복 조회 등은 후속 작업이다.
