...
리포트의 `date`는 리포트를 생성한 실행일이 아니라 데이터 기준 거래일입니다.

- 지표·신호·수급을 실제 관측일 순으로 정렬하고 각 입력의 최신 유효일 중 가장 이른 날짜를 공통 기준일(`reference_date`)로 사용합니다.
- 각 입력에서 공통 기준일 이하의 최신 행을 선택합니다. 기준일보다 미래인 행은 지표의 이전 종가 계산에도 포함하지 않습니다.
- 세 입력은 필수입니다. 빈 입력·유효 날짜 없음·기준일 이하 행 없음은 `MarketReportDataError`로 처리하며 리포트를 저장하지 않습니다.
- CSV에서 정수 또는 소수형으로 읽힌 8자리 날짜는 `YYYYMMDD` 형식으로 해석합니다. ISO 문자열과 datetime 입력도 지원합니다.
- 날짜가 없는 상세 수급은 같은 파일명의 `investor_daily_summary`와 연결합니다. 날짜별 수급값이 있으면 해당 summary 행을 사용하며, 수급 컬럼 일부가 없다고 상세 값으로 대체하지 않습니다.
- summary에 날짜만 있는 호환 경로에서는 단일 상세 행에만 최신 관측일을 연결합니다. 날짜 없는 다중 상세 행, 유효한 summary 없음 등 관측일을 확정할 수 없는 입력은 오류와 데이터 갱신 안내를 제공합니다.
- 파일명에 포함된 요청일을 관측일로 사용하지 않습니다. 일요일에 수집한 파일도 실제 관측일이 금요일이면 금요일 수급을 사용합니다.
- `indicator_date`, `signal_date`, `investor_date`는 실제 선택된 관측일입니다. 각 `*_data_status`가 `aligned`면 공통 기준일과 같고, `lagging`이면 더 과거의 관측값입니다.
- 저장소의 `as_of_date`는 날짜 컬럼 전체의 최대 날짜를 사용합니다. KIS 일봉 응답처럼 최신 거래일이 첫 행으로 내려오는 경우에도 마지막 행이 아니라 가장 큰 날짜가 기준일입니다.

따라서 `데이터 수집`만 실행한 뒤 바로 `리포트 생성`을 누르면 새 가격 데이터가 리포트에 반영되지 않을 수 있습니다. 최신 수집분을 반영하려면 `지표 계산 -> 신호 생성 -> 리포트 생성`까지 이어서 실행하거나 대시보드의 `전체 파이프라인`을 사용합니다.

수집이 모두 실패하면 전체 파이프라인은 후속 지표/신호/리포트 생성을 진행하지 않고, "리포트를 갱신하지 않았습니다" 메시지를 표시합니다.
...
| `date` | 리포트 기준 거래일 | `2026-05-14` |
| `reference_date` | 모든 필수 입력에 적용한 공통 기준일 | `2026-05-14` |
| `indicator_date`, `signal_date`, `investor_date` | 실제 선택한 입력별 관측일 | `2026-05-14` |
| `*_data_status` | 공통 기준일과 일치하거나 지연됨 | `aligned`, `lagging` |
| `close` | 종가 | `72000` |
| `ma_5` | 5일 이동평균 | `71500` |
...
>
> 상위 종합 의견과 전략별 신호는 서로 다를 수 있습니다. 대시보드에서 방향이 엇갈린다고 표시되면 어느 하나만으로 매수·매도 결정을 내리지 말고, 기준일과 각 판단 근거를 함께 확인합니다.
...
- [run_market_report.py](../../src/invest_bot/jobs/run_market_report.py)
- [discord_report_notifier.py](../../src/invest_bot/jobs/discord_report_notifier.py)
- [indicator_guide.md](indicator_guide.md)
