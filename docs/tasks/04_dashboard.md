...
- 홈은 오늘의 투자 브리핑, 오늘 볼 만한 종목, 다음 행동, quick-start 순서로 종목 중심 확인 흐름을 안내
...
- 홈 진입 시 관심종목의 저장 기준일을 확인하고 최신 거래일까지 부족한 종목만 수집한 뒤 지표, 신호, 리포트를 갱신하여 홈의 기준일과 추세에 반영함
- 홈 관심종목 최신화 검사 중에는 등록 종목 수와 처리 내용을 status로 표시하고, 이미 최신이면 기존 snapshot을 재사용함
- KIS `search-stock-info`처럼 선택적 종목 기본정보 조회가 실패해도 해당 오류를 격리하고 가격·수급·지표·신호·리포트 갱신은 계속 진행함
- 전역 글꼴 스타일을 변경할 때 Streamlit `stIconMaterial`의 Material Symbols 폰트와 ligature 설정을 반드시 보존해 status/expander 화살표 이름이 일반 텍스트로 노출되지 않도록 함
